...
## Import
  ...
  (require result [result-> result->> let-result match-result try-result])
  (import result [ok err ok? err? result? as-result
                  unwrap unwrap-or map-ok map-err raise-if-err])
"
...
(defn result? [x]
  "True if x is a Result dict (has :ok key)."
  (and (isinstance x dict) (in "ok" x)))

(defn ok? [r]
  "True if r is a successful Result."
  (and (isinstance r dict) (= (.get r "ok") True)))

(defn err? [r]
  "True if r is a failed Result."
  (and (isinstance r dict) (= (.get r "ok") False)))
...
(defmacro result-> [expr #* forms]
  ...
  Example:
    (result-> raw-input
              ...
              (validate-range 0 100)  ; int -> Result[int]  (value threaded as 1st arg)
  ...
  (if (= (len forms) 0)
    ...
    (let [r (hy.gensym "r")]
      `(let [~r (as-result ~expr)]
         (if (ok? ~r)
           (result-> (-> (get ~r "value") ~(get forms 0)) ~@(cut forms 1 None))
           ~r)))))
...
(defmacro match-result [result ok-sym ok-body err-sym err-body]
  ...
  (let [r (hy.gensym "r")]
    `(let [~r ~result]
       (if (ok? ~r)
         (let [~ok-sym (get ~r "value")] ~ok-body)
         (let [~err-sym (get ~r "error")] ~err-body)))))
...
(defn unwrap [result]
  ...
  Only call this when you have already checked ok?, or in tests,
...
(defn map-err [f result]
  ...
  (if (err? result)
    {"ok" False "error" (f (get result "error"))}
    result))
