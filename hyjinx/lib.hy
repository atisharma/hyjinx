...
(import hyjinx.result [ok err unwrap unwrap-or map-ok collect-results])
...
(defn pos? [x]
  ...
  ;; TODO: Rename to `is-pos` in next major version; keep `pos?` as alias
  (>= x 0))
  
(defn neg? [x]
  "x is negative definite (excludes zero)."
  ;; TODO: Rename to `is-neg` in next major version; keep `neg?` as alias
  (< x 0))

(defn zero? [x]
  "x is exactly (integer) zero."
  ;; TODO: Rename to `is-zero` in next major version; keep `zero?` as alias
  (= x 0))

;; Aliases for idiomatic Hy naming (is-* instead of *?)
(setv is-pos pos?)
(setv is-neg neg?)
(setv is-zero zero?)
