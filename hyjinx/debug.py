...
Commands added to pdb:
    macroexpand (form ...)  - Expand a Hy form fully
    macroexpand_1 (form ..) - Single-step macro expansion
    macros                  - List macros in current module/scope
    hy_repr <expr>          - Show Hy representation of a Python expression
...
Usage:
    >>> import hyjinx.debug
    >>> # breakpoint() now uses HyPdb automatically

    Or explicitly:
    >>> from hyjinx.debug import HyPdb, set_trace
    >>> set_trace()  # Enter debugger with HyPdb
"""
...
class HyPdb(pdb.Pdb):
    ...
    Adds commands for macro expansion and inspection within the debugger.
    Automatically installed as sys.breakpointhook when hyjinx.debug is imported.
    """
